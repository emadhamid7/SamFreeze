        ...
        apps.filter { it.packageName !in hidden }
            .map { appInfo ->
                ...
                AppInfo(
                    ...
                    apkPath = apkDirOf(appInfo)
                )
            }.sortedBy { it.label.lowercase() }
    }

    /**
     * Directory the APK actually lives in (e.g. /system/priv-app/Foo), derived
     * from sourceDir with the base.apk filename stripped off — that's the part
     * that tells the user "where the app is from" (/system/app, /product, /data/app, etc).
     */
    private fun apkDirOf(appInfo: ApplicationInfo): String? {
        val source = appInfo.sourceDir ?: return null
        return try {
